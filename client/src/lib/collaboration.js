...
export const USER_COLORS = COLORS;

export function createCollaboration(roomId, userProfile, isAI = false) {
    ...
    provider.awareness.setLocalStateField('user', {
        ...
        color: isAI ? '#a78bfa' : userProfile.color,
        avatar: userProfile.avatar || null,
    });

    const filesystem = ydoc.getMap('filesystem');
    const terminal = ydoc.getMap('terminal');
    const folderColors = ydoc.getMap('folderColors');

    return { ydoc, provider, filesystem, terminal, folderColors, awareness: provider.awareness, color: userProfile.color };
}
    ...
    awareness.getStates().forEach((state, clientId) => {
        if (state.user) {
            users.push({
                ...
                avatar: state.user.avatar || { type: 'letters', data: null },
